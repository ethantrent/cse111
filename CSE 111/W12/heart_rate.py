...
def main():
    ...
    frm_main.master.title("Heart Rate") # Set the title of the window.
...
def populate_main_window(frm_main):
    ...
    lbl_age = Label(frm_main, text="Age (12 - 90):") # Create a label widget.

    # Create an integer entry box where the user will enter her age.
    ent_age = IntEntry(frm_main, width=4, lower_bound=12, upper_bound=90) # Create an integer entry box widget.

    # Create a label that displays "years"
    lbl_age_units = Label(frm_main, text="years") # Create a label widget.

    # Create a label that displays "Rates:"
    lbl_rates = Label(frm_main, text="Rates:") # Create a label widget.

    # Create labels that will display the results.
    lbl_slow = Label(frm_main, width=3) # Create a label widget.
    lbl_fast = Label(frm_main, width=3) # Create a label widget.
    lbl_rate_units = Label(frm_main, text="beats/minute") # Create a label widget.

    # Create the Clear button.
    btn_clear = Button(frm_main, text="Clear") # Create a button widget.

    # Layout all the labels, entry boxes, and buttons in a grid.
    lbl_age.grid(      row=0, column=0, padx=3, pady=3) # Layout the label widget.
    ent_age.grid(      row=0, column=1, padx=3, pady=3) # Layout the integer entry box widget.
    lbl_age_units.grid(row=0, column=2, padx=0, pady=3) # Layout the label widget.

    lbl_rates.grid(     row=1, column=0, padx=(30,3), pady=3) # Layout the label widget.
    lbl_slow.grid(      row=1, column=1, padx=3, pady=3) # Layout the label widget.
    lbl_fast.grid(      row=1, column=2, padx=3, pady=3) # Layout the label widget.
    lbl_rate_units.grid(row=1, column=3, padx=0, pady=3) # Layout the label widget.

    btn_clear.grid(row=2, column=0, padx=3, pady=3, columnspan=4, sticky="w") # Layout the button widget.
